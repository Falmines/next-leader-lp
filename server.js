...
Terima kasih sudah mendaftar. Tim Next Leader akan menghubungi Anda untuk informasi berikutnya.`;

  const customerEmailHtml = `
    <h2>Pembayaran Berhasil</h2>
    <p>Halo <b>${order.customer.name}</b>,</p>
    <p>Pembayaran Anda untuk <b>Business Presentation Next Leader</b> sudah berhasil.</p>
    <p><b>Order ID:</b> ${order.orderId}</p>
    <p><b>Paket:</b> ${order.packageName}</p>
    <p><b>Total:</b> Rp${formatRupiah(order.amount)}</p>
    <p>Tim Next Leader akan menghubungi Anda untuk informasi berikutnya.</p>
  `;

  await sendEmail({
    to: order.customer.email,
    subject: "Pembayaran Berhasil - Next Leader",
...
app.post("/api/create-transaction", async (req, res) => {
  try {
    ...
    console.error(error);
    res.status(500).json({
      message: "Gagal membuat transaksi payment gateway",
      error: error.message
    });
  }
});
...
app.post("/api/midtrans/notification", async (req, res) => {
  try {
    ...
    if (existingOrder && status === "paid" && !existingOrder.successNotificationSent) {
      await sendPaymentSuccessNotification(existingOrder);
